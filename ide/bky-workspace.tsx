...
export function BkyWorkspace() {
  ...
  // Funcao para atualizar a screen com o conteudo de blocos (dispara auto-sync global)
  const updateScreenBlocks = useCallback((screenName: string, bkyContent: string) => {
  ...
  // Funcao para salvar blocos na screen atual (dispara auto-sync global automaticamente)
  const saveBlocksToScreen = useCallback((ws: Blockly.WorkspaceSvg) => {
    ...
    // Atualizar diretamente no objeto screens (isso dispara o auto-sync global)
    const { currentScreenName } = useIDEStore.getState()
    if (currentScreenName) {
      updateScreenBlocks(currentScreenName, blocksJson)
    ...
    setIsSaving(false)
  }, [setCurrentBkyContent, updateScreenBlocks])
